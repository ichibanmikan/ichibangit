...
class library_system_user : public user_admini{
    ...
    public:
		...
		void showUsers(){
			cout << "当前显示第1页，查看下一页请按0，查看上一页请按1，退出请按2" << endl;
			for (int i=0; i<vecUS.size()/2; i++){
			...
			while(true){
				...
				cin >> a;
				if(a==0){
					cout << "当前显示第2页，查看下一页请按0，查看上一页请按1，退出请按2" << endl;
					for (int i=vecUS.size()/2; i<vecUS.size(); i++){
						cout << vecUS[i].getName() << ' ' << vecUS[i].account << endl;
					}
				}
				if(a==1){
		...
		void forgetPassword(string account_str, string name, int temp){
			...
			cout << "抱歉，未查找到相关信息,无法找回并修改密码，请重新输入密码登录" << endl;
		}
		...
		int findStudent_Name(string name_str){
            for (int i=0; i<vecUS.size(); i++){
				if(vecUS[i].getName()==name_str){
					return i;
				}
			}
			return -1;
        }//根据姓名查找学生在库中的位置
		int findStudent_Account(string Account_str){
			for (int i=0; i<vecUS.size(); i++){
				if(vecUS[i].account==Account_str){
					return i;
				}
			}
			return -1;
